...
- 공개 노드: **22개** (콘텐츠 페이지 18 + 데이터베이스 페이지 4)
- 데이터베이스: **5개 / 351행**
- 뷰: **12개**
- 첨부 파일: **90개** (`file` 블록 23 + `image` 블록 5 + DB 속성 62). 미해결 0
...
│   │   └── (토글 "이력" 내부 — 여기서도 크롤러가 놓쳤다)
│   │       ├── 회칙 (2024.11.29~2025.09.05) /3042f5f5b3ac81a2a196ef1dc174b66c   70블록
│   │       └── 회칙 (2025.09.05~2026.02.11) /3042f5f5b3ac81a9bceecf89b9346009   76블록
...
└── 데이터베이스                            /3042f5f5b3ac81f4923bc6642b5e3f1f
    ├── 활동 기록 (DB)                      /3042f5f5b3ac81f0b9ace74f33386d43
...
`활동 기록` DB의 실제 `parent_id`는 `데이터베이스`다. 루트는 인라인 캘린더 뷰로 *표시*만 한다.

ancestor-path에 뜨는 제목 없는 중간 노드는 페이지가 아니라 레이아웃 블록이다:
`...81819ea9...`가 `column_list`, 그 아래 `...812b9b8d...`와 `...8180be25...`가 `column`.
별도 URL로 노출되지 않는다.

> 🔴 **토글 lazy 로딩을 두 번 놓쳤다.** `기타 활동 자료`에서 3페이지, `회칙 → 이력`에서
> 2페이지. loadPageChunk는 토글 자식을 안 내려준다. **모든 토글에 대해 자식 id로 재귀
> 재조회하지 않으면 반드시 누락된다.** 특히 회칙 개정본 2건은 이 문서 최대 작업 항목
> (`/about/charter`) 한복판에 있다.
...
| 회칙 | 정적 장문 | 5장, **조문 11개**(header 5 / sub_header 11 / numbered_list 34) + 토글 "이력"(개정본 2건).<br>🔴 **제7조 → 제9조로 건너뛴다. 제8조가 없다.** 옮기기 전에 동아리에 확인할 것 |
...
> ⚠️ 위 분포는 **토글 자식을 놓친 1차 크롤 기준**이라 과소 집계다. 회칙 개정본 2건(70+76블록),
> 토글 내부 이미지 5개, `bulleted_list`·`image`·`button`이 빠져 있다. 실행 시 재집계할 것.

`code` 블록 1개(선거 공약 LaTeX 양식).

> **정정**: 초판은 "이미지가 인라인 마크다운(`![]()`)이라 `image` 블록으로 안 잡힌다"고 썼다.
> 틀렸다. 홍보 포스터는 정상적인 **`image` 블록**이다
> (`3042f5f5-b3ac-817c-a3bd-c47391bfe9ae`, source `attachment:c2f5a991-...:2025-2_홍보_포스터.png`).
> `![]()`는 Notion MCP가 **출력을 마크다운으로 렌더한 형식**일 뿐 원본 블록 타입이 아니었다.
> 열거기에 마크다운 파서는 필요 없다. **필요한 건 토글 재귀뿐이다.**
...
|---|---|---|---|
| 회원 | `3042f5f5-b3ac-81e3-9641-000b7950b6f9` | **231** | 활동 회원(table), 전체 회원(table, 필터 有), 개인 프로젝트(board) |
...
`개인 프로젝트` 보드 뷰는 회원 DB를 대상으로 한 세 번째 뷰다(별도 collection 아님).
`group_by`가 null이라 그룹 기준은 미확인 — 실행 시 확인할 것.
...
## 4. 첨부 파일 인벤토리 (90개, 미해결 0)

### 블록 첨부 (28)

`file` 블록 **23** — 부모 페이지 기준 실측:
...
| 자연대 Integration Bee | 2 | `적분 문제.pdf`, `적분 문제 (답).pdf` |

`image` 블록 **5**:

| 위치 | 개수 | 파일 |
|---|---|---|
| 갤러리 → "동아리 로고" 토글 | 4 | `로고_1.png`, `로고_2.png`, `스티커.png`, `아이콘_3.png` |
| 홍보 자료 → "2025-2" 토글 | 1 | `2025-2_홍보_포스터.png` |
...
**합계 90.** 초판이 "미해결"로 남겼던 항목은 전부 해소됐다.

### 🔴 파일 URL의 성질 — 접근 경로마다 다르다

두 형태가 공존한다. **초판은 한쪽만 보고 "5분"을 파이프라인 설계 전제로 삼았다.**

| 경로 | 반환 형태 | 유효기간 |
|---|---|---|
| v3 `getSignedFileUrls` | `file.notion.so/...?expirationTimestamp=&signature=&spaceId=&table=` | **수 시간** (실측: 정시 경계까지, 약 6.5h) |
| MCP/렌더 경로 | `prod-files-secure.s3.us-west-2.amazonaws.com/...?X-Amz-Expires=300` | **300초** |
| 블록 `source` 원문 | `attachment:<uuid>:<filename>` (23개 중 13개) 또는 서명 없는 S3 URL (10개) | 서명 없는 쪽은 **HTTP 403** |

결론은 어느 쪽이든 같다: **URL을 저장하거나 핫링크하지 말고, 다운로드 후 재업로드한다.**
다만 "5분 안에 소비" 같은 촉박한 제약을 설계에 넣을 필요는 없다.
...
| 역대 회장단 | 4기수 8명의 실명 · 학부 · 학번 · **개인 이메일 7건**(1명 중복 재임) |
