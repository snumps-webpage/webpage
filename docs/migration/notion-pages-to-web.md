...
범위: **snumps.notion.site에 로그인 없이 노출된 콘텐츠 22노드**를 이 SvelteKit 앱의
실제 웹페이지로 옮긴다. `관리자 전용`은 비공개이므로 이 문서 범위 밖이지만,
Notion 완전 대체 대상이므로 [`notion-db-to-s3.md`](./notion-db-to-s3.md)가 다룬다.

> **확정 결정 (2026-08-23)**: Notion을 온전히 대체한다 — 편집도 서빙도 전부 옮긴다.
> 따라서 §3-1의 "1회 변환"은 **"이후 이 콘텐츠를 고칠 방법을 앱이 제공해야 한다"**를 뜻한다.
> Notion으로 돌아가 고치는 선택지는 없다.
...
| 동아리 문서 | `/about` 허브 | 정적 | — |
| 회칙 | `/about/charter` | 정적 장문 | **레포 내 마크다운** (D1 — 저장소 조회 없음) |
| ├ 회칙 개정본 2건 | `/about/charter/history/<기간>` | 정적 장문 | 마크다운 (70·76블록) |
...
**정적 콘텐츠는 이주 시점에 1회 변환한다.**

> 🔴 **변환 후 누가 어떻게 고치는가가 결정돼야 이 항목이 끝난다.** Notion이 사라지므로
> 회칙 개정, 회장단 교체, 홍보문 추가를 할 방법이 반드시 있어야 한다.
> 선택지와 트레이드오프는 [`notion-db-to-s3.md`](./notion-db-to-s3.md) §5-3에 정리했고,
> 결정은 그쪽 §8-1이다. **레포 PR로 간다면 "임원진이 GitHub PR을 연다"가 합의된
> 운영 절차여야 한다** — 사고로 그렇게 되면 안 된다.
...
> 🔴 **단 이 재사용은 한시적이다.** Notion 완전 대체가 확정됐으므로 최종 출처는
> S3 테이블(`tables/*.json`)이다. `withCache(key, ttl, fetcher)` 인터페이스는 유지되고
> 페처만 바뀌므로(`notion-db-to-s3.md` §2-6) 페이지 쪽 코드는 대체로 영향을 안 받지만,
> **DB 이주 B4(읽기 경로 전환) 이후에 붙이는 편이 두 번 고치지 않는다.**
...
| 항목 | 현 상태 | 권고 |
|---|---|---|
| 회원 231명 이름·학과·가입일 | 전체 공개 | **확정(D2): 현행 공개 범위 그대로 유지.** 이미 그렇게 운영해 왔고 새로 감추지 않는다. 단 저장은 비공개 버킷 |
...
블록 첨부 28개(`file` 23 + `image` 5) + DB 첨부 62개 = **90개**, 미해결 0. Notion presigned URL은 **300초 만료**라 DB에
...
회칙이 최대 작업량인데 첨부가 0개다. **자산 파이프라인을 기다릴 이유가 없다.**
게다가 회칙은 현행본 + **개정본 2건(70·76블록)**으로 실제 분량이 초판 추정의 3배다.
초판이 "미해결"로 걱정하던 동아리 로고는 4개 이미지로 해소돼 자산 단계의 블로커도 아니다.
...
앱에는 `/signup`이 있다. **Notion이 사라지면 그 홍보문도 앱이 서빙하므로,
안내 링크를 `/signup`으로 바꾸는 것은 이주 작업의 일부다** — 선택이 아니다.
Google Form 자체의 폐기 여부(기존 응답 처리 포함)만 별도 판단.
...
| 항목 | 내용 |
|---|---|
| 리다이렉트 | notion.site는 커스텀 리다이렉트를 못 건다. Notion 루트를 "새 사이트로 이동" 안내 페이지로 바꾸고 나머지는 순차 비공개 전환. **워크스페이스는 즉시 삭제하지 말고 아카이브로 보존** |
...
2. ~~회원 DB 공개 범위~~ → **확정: 현행 공개 범위 유지** (D2)
3. **개인 연락처** — 임원 전화번호·역대 회장단 이메일 공개 여부 (본인 동의 확인 필요)
4. **가입 창구** — Google Form 폐기하고 `/signup` 일원화가 맞는지
5. **마크다운 도구** — mdsvex 채택 여부
6. ~~**Notion 존치 여부**~~ → **확정: 완전 이탈.** 대신 정적 문서 편집 수단이
   결정 대상으로 승격됐다 (`notion-db-to-s3.md` §5-3 / §8-1)
7. **전환 시점** — 신·구 병행 기간을 둘지, 일시 전환할지.
   단 Notion을 닫는 시점은 **편집 UI 완성 이후**여야 한다 (`notion-db-to-s3.md` §9 첫 행)
