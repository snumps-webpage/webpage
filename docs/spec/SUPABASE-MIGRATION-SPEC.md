...
> | S10 | **자동 메일 3계층 분리** (2026-08-31) — 이벤트 카탈로그(코드 고정: 발생 시점·제공 변수·허용 수신자·기본 규칙, `mail/events.ts`) / 발송 규칙(`mail-rules` 테이블: 이벤트→템플릿→수신자, 이벤트별 행 존재 시 전체 진실·없으면 기본 규칙, 첫 편집 시 실체화) / 템플릿(`mail-templates`: 기본값 오버라이드 + 커스텀). 수신자는 닫힌 해석기 4종(당사자/관리자/회장단/수신동의 전 회원). 관리는 `/admin/mail`. 발송 실패는 본 동작을 막지 않음(§5-7 유지) |
