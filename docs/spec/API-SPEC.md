# SNUMPS 웹페이지 — API 명세 (v0.4)

> **전제**: [`FUNCTIONAL-SPEC.md`](./FUNCTIONAL-SPEC.md) v0.7.
...
- `mutate`: GET(ETag) → fn → PUT If-Match, 실패 시 재읽기 재시도(지수 백오프, 최대 5회) → `WRITE_CONFLICT`
...
```jsonc
{
  ...
  "attendeeIds": ["ULID"],
  "sourceRequestId": "ULID | null"      // §1-6. 승인·회차 생성이 만든 경우 원 신청/회차 근거
}
```
...
| `GET /members` | PUB-15 | name·department·joinedAt·roles (D2 범위) | ISR(60s) |
...
- 처리: `mutate(applications)` 신규(pending) → 관리자 알림 메일 (실패 시 `mailFailed`)
- 에러: `VALIDATION_FAILED`, `CONFLICT`(pending 신청 존재)
...
- 처리:
  1. `mutate(members)`: `status: withdrawn`, `withdrawal: { requestedAt: now }`
  2. **현 회장단 통지 메일** (최신 term의 회장·부회장 `publicContact`/이메일 대상) + 관리자 대시보드 표시
...
- 집행 내용:
  ...
  - `members` 행은 **이름·학과·status만 잔존** — `joinedAt`·`publicContact`·`project`·`roles` 외 인적 필드 `null` 처리.
    단 `roles`(임원 이력)는 공개 기록이므로 유지
...
### 7-1. `GET /admin` — 가입·세미나·스터디 신청(pending), 출석 큐(pending), 이벤트 전 상태
...
| `?/approveAttendance` | ADM-03 | 검증: 이벤트·활동 실재 (dangling → `NOT_FOUND`). `activities.attendeeIds` 추가 → queue `approved`. 캐시: `activities_*`, `user_activities_<memberId>` |
| `?/rejectAttendance` / `?/deleteAttendanceRecord` | ADM-03 | **approved 행에 적용 시 역반영** — `attendeeIds`에서 제거 후 상태 변경/삭제. 캐시 동일 |
| `?/updateAttendanceTime` | ADM-03 | 시각 수정 |
...
- 처리:
  ...
  3. **탈퇴 익명화 집행** (MEM-07): §4-7 자동 삭제 규칙 충족 회원 → private-info 삭제 +
     members 익명화 + 감사 로그. 멱등 (이미 익명화된 행은 대상 조건 불충족)
- 응답: `{ success: true, expired: n, generated: n, anonymized: n }`

### 8-2. 업로드 — SYS-03

- `POST /api/uploads/presign` — 가드 `ensureAdmin`
- 입력: `{ purpose, filename, contentType, size }`. purpose별 타입·크기 상한 (이미지 10MB, PDF 50MB)
- 응답: `{ uploadUrl, s3Key }` (presigned PUT 10분, 키에 해시 포함)
...
- **공개 응답 감사**: §3 전 로드 — PII·운영 필드 부재 스냅샷. `publicContact` 외 연락처 부재. **`/` 세션 분기: 게스트 캐시에 회원 데이터 미혼입**
