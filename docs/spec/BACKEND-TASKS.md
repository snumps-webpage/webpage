...
| BE-41 | 탈퇴 수명주기 | §4-7 — 삼중 확인 서버 검증, 회장단 통지(메일 템플릿 자체 포함 — BE-45 무관), `/withdraw/pending` 철회, 보존 집행 데이터 준비, 감사 로그. 🔶 **자동 익명화 단계는 구현 보류** (추가 검토 중 — 크론 미탑재, 유예 경과 시 상태 유지) | BE-31, BE-13, BE-35 |
...
| BE-72 | 탈퇴 수명주기 E2E (신청→통지→보존/철회. 🔶 익명화 구간은 보류 해제 시 추가) | BE-41 |
