<script lang="ts">
  ...
  const activities = $derived.by(() => {
    ...
    return data.activities.filter((activity) =>
      !normalized || [activity.title, activity.type, dateOf(activity)].some((value) => value.toLocaleLowerCase("ko-KR").includes(normalized)),
    );
  });
  ...
  function dateToStart(originalDate?: string) {
    return (event: FormDataEvent) => {
      ...
      if (picked && picked !== originalDate) event.formData.set("start", `${picked}T00:00`);
    };
  }
...
<article class="paper-document activity-admin-paper">
  ...
  {#if form?.success && form.operation}<p class="paper-status-note success" role="status">{operationLabel[form.operation as string]}</p>{/if}
  {#if form?.error === "CONFLICT"}<p class="paper-status-note error" role="alert">연결된 이벤트가 있어 삭제할 수 없습니다. 먼저 이벤트와 출석 큐를 정리해 주세요.</p>{/if}

  <section class="create-panel">
    <h2>1. 새 활동 기록</h2>
    <form method="POST" action="?/create" class="record-form" onformdata={dateToStart()}>
      <label><span class="paper-label">활동명</span><input name="title" value="" /></label>
      <label><span class="paper-label">유형</span><select name="type">{#each data.activityTypes as type (type)}<option value={type} selected={type === "세미나"}>{type}</option>{/each}</select></label>
      <label><span class="paper-label">날짜</span><input type="date" name="date" value={today} /></label>
    ...
    <div class="record-list">
      {#each activities as activity (activity.id)}
        <details class="record-card">
          <summary><div><span>{dateOf(activity)} · {activity.type}</span><strong>{activity.title}</strong></div><small>{activity.attendeeIds.length}명 참석 · 이벤트 {activity.linkedEventIds.length}개</small></summary>
          <div class="record-body">
            <form method="POST" action="?/update" class="record-form" onformdata={dateToStart(dateOf(activity))}><input type="hidden" name="id" value={activity.id} /><label><span class="paper-label">활동명</span><input name="title" value={activity.title} /></label><label><span class="paper-label">유형</span><select name="type">{#each data.activityTypes as type (type)}<option value={type} selected={type === activity.type}>{type}</option>{/each}</select></label><label><span class="paper-label">날짜</span><input type="date" name="date" value={dateOf(activity)} /></label><button class="paper-btn" type="submit">기본 정보 수정</button></form>
